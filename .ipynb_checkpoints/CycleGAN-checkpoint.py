...
class CycleGAN(tf.keras.Model):
    ...
    def __init__(self, output_channels, reconst_weight=1, train_summary_writer=None):
        '''
...
#         self.color_d_weight = color_d_weight
    ...
    def get_name(self):
        return f"reconst_{self.reconst_weight}"
    ...
    def save_generated_results(self, landscape, filename):
        ...
        display_list = [landscape[0], fake_camo[0], patched_landscape[0]]
        title = ['Original Landscape', 'Generated Camo', 'Patched Landscape']
    ...
    def save_benchmark_results(self, epoch, benchmark_landscapes, save_image_path, num_benchmarks=10):
        ...
        for i, benchmark in enumerate(benchmark_landscapes.take(num_benchmarks)):
            benchmark_results_path = f'{save_image_path}/epoch_{epoch}_camo.png'
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if self.train_summary_writer and (step_i+1) % log_freq == 0:
                with self.train_summary_writer.as_default():
                    ...
                    tf.summary.scalar('total_gen_f_loss', total_gen_f_loss, step=step_i)
